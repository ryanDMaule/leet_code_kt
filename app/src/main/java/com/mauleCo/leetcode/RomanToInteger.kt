package com.mauleCo.leetcode

@Suppress("KotlinConstantConditions")
class RomanToInteger {

    /**
     * DESCRIPTION
     *
     * Roman numerals are represented by seven different symbols: I, V, X, L, C, D and M.
     *
     * Symbol       Value
     * I             1
     * V             5
     * X             10
     * L             50
     * C             100
     * D             500
     * M             1000
     * For example, 2 is written as II in Roman numeral, just two ones added together. 12 is
     * written as XII, which is simply X + II. The number 27 is written as XXVII, which
     * is XX + V + II.
     *
     * Roman numerals are usually written largest to smallest from left to right. However, the
     * numeral for four is not IIII. Instead, the number four is written as IV. Because the
     * one is before the five we subtract it making four. The same principle applies to the number
     * nine, which is written as IX. There are six instances where subtraction is used:
     *
     * I can be placed before V (5) and X (10) to make 4 and 9.
     * X can be placed before L (50) and C (100) to make 40 and 90.
     * C can be placed before D (500) and M (1000) to make 400 and 900.
     * Given a roman numeral, convert it to an integer.
     */

    private val numeralMap = hashMapOf(
        'I' to 1,
        'V' to 5,
        'X' to 10,
        'L' to 50,
        'C' to 100,
        'D' to 500,
        'M' to 1000,
    )

    fun romanToInt(s: String): Int {
        val numeralArray = s.toCharArray()
        var index = 0
        var intTotal = 0

        while (index <= numeralArray.size){
            when(numeralArray[index]){
                'I' -> {
                    try {
                        if (numeralArray[index+1] == 'V'){
                            intTotal += 4
                            index++
                        } else if (numeralArray[index+1] == 'X') {
                            intTotal += 9
                            index++
                        } else {
                            intTotal += 1
                        }
                    } catch (e : Exception) {
                        intTotal += 1
                    }
                }
                'X' -> {
                    try {
                        if (numeralArray[index+1] == 'L'){
                            intTotal += 40
                            index++
                        } else if (numeralArray[index+1] == 'C') {
                            intTotal += 90
                            index++
                        } else {
                            intTotal += 10
                        }
                    } catch (e : Exception) {
                        intTotal += 10
                    }
                }
                'C' -> {
                    try {
                        if (numeralArray[index+1] == 'D'){
                            intTotal += 400
                            index++
                        } else if (numeralArray[index+1] == 'M') {
                            intTotal += 900
                            index++
                        } else {
                            intTotal += 100
                        }
                    } catch (e : Exception) {
                        intTotal += 100
                    }
                }
                'V', 'L', 'D', 'M' -> {
                    intTotal += numeralMap.getValue(numeralArray[index])
                }
            }

            index++
            if (index > numeralArray.size -1){
                break
            }
        }

        return intTotal
    }

}